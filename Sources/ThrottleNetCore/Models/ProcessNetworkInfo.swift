...
public struct ProcessNetworkInfo: Identifiable, Equatable {
  ...
  public static func == (lhs: ProcessNetworkInfo, rhs: ProcessNetworkInfo) -> Bool {
    return lhs.pid == rhs.pid && lhs.downloadBytesPerSec == rhs.downloadBytesPerSec
      && lhs.uploadBytesPerSec == rhs.uploadBytesPerSec && lhs.totalBytesIn == rhs.totalBytesIn
      && lhs.totalBytesOut == rhs.totalBytesOut && lhs.throttleConfig == rhs.throttleConfig
      && lhs.persistentRule == rhs.persistentRule && lhs.activeSocketCount == rhs.activeSocketCount
  }
}
