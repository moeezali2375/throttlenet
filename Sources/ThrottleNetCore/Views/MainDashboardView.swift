        ...
        VStack(spacing: 0) {
            ...
            if list.isEmpty {
                VStack(spacing: 12) {
                    ...
                    Text(viewModel.searchQuery.isEmpty ? "No active network processes detected" : "No processes matching \"\(viewModel.searchQuery)\"")
                        .font(.system(size: 13, weight: .medium))
                        .foregroundColor(.secondary)
                    
                    Text("ThrottleNet is continuously monitoring network sockets via macOS diagnostic subsystem.")
                        .font(.system(size: 11))
            ...
            HStack(spacing: 12) {
                HStack(spacing: 6) {
                    ...
                    Text(viewModel.monitor.isMonitoring ? "Monitoring (\(list.count) processes)" : "Monitoring Paused")
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                }
                ...
                Button(action: {
                    ...
                    Label(viewModel.monitor.isMonitoring ? "Pause" : "Resume",
