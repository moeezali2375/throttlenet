...
public struct ThrottleSheetView: View {
  ...
  public init(process: ProcessNetworkInfo, viewModel: DashboardViewModel) {
    ...
    _rememberAsAutoRule = State(
      initialValue: persistentRule != nil ? persistentRule!.autoApplyOnLaunch : true)
  }
    ...
    VStack(alignment: .leading, spacing: 16) {
                ...
                ForEach(downloadPresets, id: \.kbps) { preset in
                  Button(action: {
                    ...
                    Text(preset.label)
                      .font(
                        .system(
                          size: 10, weight: downloadLimitKBps == preset.kbps ? .bold : .regular))
                  }
              ...
              HStack(spacing: 6) {
                ForEach(uploadPresets, id: \.kbps) { preset in
                  Button(action: {
                    ...
                    Text(preset.label)
                      .font(
                        .system(size: 10, weight: uploadLimitKBps == preset.kbps ? .bold : .regular)
                      )
                  }
                  .buttonStyle(.bordered)
      ...
      HStack(alignment: .top, spacing: 10) {
        ...
        VStack(alignment: .leading, spacing: 2) {
          ...
          Text(
            "ThrottleNet will automatically re-apply this limit whenever \(process.displayName) starts or runs in the background."
          )
          .font(.system(size: 10))
          .foregroundColor(.secondary)
        }
